...
interface EmptyBasketTypes {
  basketSwitcher: Function;
}

const EmptyBasket = ({ basketSwitcher }: EmptyBasketTypes) => {
  return (
    <div className="basket-page__empty">
      <h2>
        The shopping cart is empty <img src={smile} alt="" width="30px" height="30px" />{" "}
      </h2>
      <p>Maybe, You didn't order any pizza</p>
      <p>to do it go to main page</p>
      <div className="basket-page__empty-img">
        <img src={emptyBasket} alt="" />
      </div>
      <button onClick={() => basketSwitcher()}>Go back</button>
    </div>
  );
};
